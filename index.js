...
function searchMeal() {
    ...
    if (!userInput) {
        resultContainer.innerHTML = `<h3 class="msg">Input Field Cannot Be Empty</h3>`;
        return;
    }
    ...
    fetch(apiUrl + userInput).then((response) => response.json()).then((data) => {
        ...
        if (!meal) {
            resultContainer.innerHTML = `<h3 class='msg'>No Meal Found, Please Try Again!</h3>`;
            return;
        }
        ...
        // Handle error
        .catch(() => {
            resultContainer.innerHTML = `<h3 class='msg'>Error fetching data!</h3>`;
        });
}
...
function getIngredients(meal) {
    ...
    for (let i = 1; i <= 20; i++) {
        ...
        if (ingredient) {
            ...
            ingredient_Html += `<li>${measure} ${ingredient}</li>`;
        }
